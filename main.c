...
/* Definitions for TaskACC */
osThreadId_t TaskACCHandle;
uint32_t TaskACCBuffer[ 512 ];
osStaticThreadDef_t TaskACCControlBlock;
const osThreadAttr_t TaskACC_attributes = {
		.name = "TaskACC",
		.cb_mem = &TaskACCControlBlock,
...
//uint16_t new_sampling_freq = 1;
extern volatile uint32_t sampling_period;  //msec
extern volatile uint32_t freq_write_flag;
extern volatile uint16_t freq_written_value;
uint32_t sampledTime; 
int bAccUpdate = 0;
/* USER CODE END PV */
...
void StartTaskACC(void *argument)
{
    ...
    uint32_t last_freq_seen = 1;
    uint32_t last_sampling_period = 1000;

    for(;;)
    {
        if (freq_written_value != last_freq_seen) {
            last_freq_seen = freq_written_value;
            last_sampling_period = 1000 / last_freq_seen;
            if (last_sampling_period < 5) last_sampling_period = 1000;
            sampledTime = HAL_GetTick();

            printf("🆕 Updated sampling to %lu Hz (period = %lu ms)\n", last_freq_seen, last_sampling_period);
        }

        uint32_t curTime = HAL_GetTick();
        if ((curTime - sampledTime) >= last_sampling_period) {
            sampledTime = curTime;
            BSP_ACCELERO_AccGetXYZ(pDataXYZ);
            bAccUpdate = 1;

            printf("[%lu ms] X: %d, Y: %d, Z: %d (interval = %lu ms)\r\n",
                   sampledTime, pDataXYZ[0], pDataXYZ[1], pDataXYZ[2], last_sampling_period);
        }

        osDelay(last_sampling_period);
    }
}
