...
#define SSID     "go get ur own wifi lol"
#define PASSWORD "Wang0207"

uint8_t RemoteIP[] = {192,168,154,210};
...
int main(void)
{
  ...
  int16_t accel_data[3];  // Array to store X, Y, Z acceleration values
  ...
  while (1) {
      if (Socket != -1) {
          ...
          // Print accelerometer values via UART
          TERMOUT("Accel: X=%d Y=%d Z=%d\n", accel_data[0], accel_data[1], accel_data[2]);

          // Format and send accelerometer data over WiFi
          char message[100];
          sprintf(message, "Accel: X=%d Y=%d Z=%d\n", accel_data[0], accel_data[1], accel_data[2]);
          ret = WIFI_SendData(Socket, (uint8_t *)message, strlen(message), &Datalen, WIFI_WRITE_TIMEOUT);
          ...
          HAL_Delay(500);  // Read and send data every 500ms
      }
  }
}
