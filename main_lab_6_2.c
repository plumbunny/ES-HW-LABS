...
osThreadId defaultTaskHandle, printHandle;
osSemaphoreId printSemaphoreHandle;
/* USER CODE BEGIN PV */
 ...
 int main(void)
{
	...
	/* USER CODE END 1 */
	 ...
	 /* add semaphores, ... */
	 	osSemaphoreDef(printSemaphore);
	 	printSemaphoreHandle = osSemaphoreCreate(osSemaphore(printSemaphore), 1);
	 ...
	 /* add threads, ... */
	 	osThreadDef(printTask, TaskPrint, osPriorityAboveNormal, 0, 128);
	 	printHandle = osThreadCreate(osThread(printTask), NULL);
